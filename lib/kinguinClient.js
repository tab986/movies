...
const KINGUIN_BASE = process.env.KINGUIN_API_BASE || 'https://gateway.kinguin.net/esa/api';// agine really why the api key is here
...
async function fetchKinguinProductById(productId) {
  const kinguinId = Number(productId);
  if (!Number.isFinite(kinguinId) || kinguinId <= 0) {
    throw new Error("Invalid Kinguin product id");
  }

  const res = await withRetry(() => client.get(`/v1/products/${kinguinId}`));
  return res.data;
}

module.exports = { client, withRetry, isoNowZ, fetchKinguinProductById };